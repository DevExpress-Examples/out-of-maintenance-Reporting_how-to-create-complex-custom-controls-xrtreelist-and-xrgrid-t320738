	...
	Friend Class XRDataControlAutoWidthCalculator
		...
		Public Overrides Sub CreateList(ByVal e As AutoWidthCalculatorArgs)
			...
			For i As Integer = 0 To count - 1
				...
				Dim minWidth As Integer = CInt(Math.Truncate(GraphicsUnitConverter.Convert(2, GraphicsDpi.HundredthsOfAnInch, containerControl.Dpi)))
				Dim maxWidth As Integer = CInt(Math.Truncate(containerControl.WidthF)) - (containerControl.VisibleHeaders.Count - 2) * CInt(Math.Truncate(GraphicsUnitConverter.Convert(2, GraphicsDpi.HundredthsOfAnInch, containerControl.Dpi)))
