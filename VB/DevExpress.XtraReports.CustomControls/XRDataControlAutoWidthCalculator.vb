	...
	Friend Class XRDataControlAutoWidthCalculator
		...
		Public Overrides Sub CreateList(ByVal e As AutoWidthCalculatorArgs)
			...
			For i As Integer = 0 To count - 1
				Dim column As XRResizableFieldHeader = CType(list(i), XRResizableFieldHeader)
				Dim minWidth As Integer = CInt(Fix(GraphicsUnitConverter.Convert(2, GraphicsDpi.HundredthsOfAnInch, containerControl.Dpi)))
				Dim maxWidth As Integer = CInt(Fix(containerControl.WidthF)) - (containerControl.VisibleHeaders.Count - 2) * CInt(Fix(GraphicsUnitConverter.Convert(2, GraphicsDpi.HundredthsOfAnInch, containerControl.Dpi)))
				Me.Objects.Add(New AutoWidthObjectInfo(column, minWidth, maxWidth, CInt(Fix(column.Width)), CInt(Fix(column.Width)), False))
			Next i
		End Sub
