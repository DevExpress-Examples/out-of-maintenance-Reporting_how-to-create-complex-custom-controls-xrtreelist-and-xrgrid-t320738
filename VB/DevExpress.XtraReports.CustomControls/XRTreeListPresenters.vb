...
Namespace DevExpress.XtraReports.CustomControls
	Friend Class XRTreeListRuntimePresenter
		...
		Protected Overrides Sub CreateBricksForRecord(ByVal record As XRDataRecord, ByVal parentBrick As PanelBrick, ByVal isHeader As Boolean, ByRef actualHeight As Single)
			...
			If args.SuppressType = NodeSuppressType.Leave Then
				...
				Dim nodeBrick As New TreeListNodeBrick(TreeList, CType(parentBrick, DataContainerBrick), isHeader)
				...
				If (Not IsDesignMode) Then
					If isHeader Then
						CType(parentBrick, DataContainerBrick).PrintCache.HeaderCache = cache
					Else
						CType(parentBrick, DataContainerBrick).PrintCache.RecordsCache.Add(cache)
					End If
				End If
			End If
		...
		Protected Overrides Function CreateBrickStyle(ByVal control As XRDataContainerControl, ByVal parentBrick As VisualBrick, ByVal valueBrick As VisualBrick, ByVal record As XRDataRecord, ByVal fieldIndex As Integer, ByVal isHeader As Boolean) As BrickStyle
			...
			If (Not isHeader) Then
				Dim printCellArgs As PrintCellEventArgs = New PrintNodeCellEventArgs(CType(record, XRTreeListNode), CType(control.VisibleHeaders(fieldIndex), XRTreeListColumn), valueBrick, style)
				TreeList.OnPrintNodeCell(CType(printCellArgs, PrintNodeCellEventArgs))
			End If
		...
		Protected Overrides Function CreateCellBrick(ByVal control As XRDataContainerControl, ByVal parentBrick As VisualBrick, ByVal record As XRDataRecord, ByVal fieldIndex As Integer, ByVal isHeader As Boolean) As VisualBrick
			...
			Dim columnWidth As Single = (CType(control.VisibleHeaders(fieldIndex), XRTreeListColumn)).Width
			Dim columnPos As Single = 0

			If fieldIndex > 0 Then
				columnPos -= (CType(record, XRTreeListNode)).Level * TreeList.NodeIndent
			End If

			For j As Integer = 0 To fieldIndex - 1
				columnPos += (CType(control.VisibleHeaders(j), XRTreeListColumn)).Width
			Next j

			Dim curColWidth As Single = If(fieldIndex = 0, columnWidth - (CType(record, XRTreeListNode)).Level * TreeList.NodeIndent, columnWidth)
		...
		Protected ReadOnly Property TreeList() As XRTreeList
			Get
				Return CType(control, XRTreeList)
			End Get
		End Property
		#End Region
	End Class
	...
	Friend Class XRTreeListDesignTimePresenter
		...
		Public Overrides Function CreateBrick(ByVal childrenBricks() As VisualBrick) As VisualBrick
			Return New DataContainerBrick(TreeList, False) With {.PrintCache = New XRDataContainerPrintCache(CType(Control, XRTreeList))}
		End Function
