	...
	Friend Class XRDataContainerControlPresenter
		...
		Protected Overridable Function GetActualBrickStyle(ByVal parentBrick As DataContainerBrick, ByVal isHeader As Boolean) As BrickStyle
			...
			If isHeader Then
				...
				If DirectCast(Control.Styles, XRDataContainerStyles).HeaderStyle IsNot Nothing Then
					ApplyStyleProperties(DirectCast(Control.Styles, XRDataContainerStyles).HeaderStyle, resultingStyle)
				End If
			Else
                resultingStyle = New XRControlStyle(_Control.fDefaultCellStyle)
				If DirectCast(Control.Styles, XRDataContainerStyles).CellStyle IsNot Nothing Then
					ApplyStyleProperties(DirectCast(Control.Styles, XRDataContainerStyles).CellStyle, resultingStyle)
				End If

				If parentBrick.PrintCache.RecordsCache.Count Mod 2 = 0 AndAlso DirectCast(Control.Styles, XRDataContainerStyles).OddCellStyle IsNot Nothing Then
					ApplyStyleProperties(DirectCast(Control.Styles, XRDataContainerStyles).OddCellStyle, resultingStyle)
				End If

				If parentBrick.PrintCache.RecordsCache.Count Mod 2 <> 0 AndAlso DirectCast(Control.Styles, XRDataContainerStyles).EvenCellStyle IsNot Nothing Then
					ApplyStyleProperties(DirectCast(Control.Styles, XRDataContainerStyles).EvenCellStyle, resultingStyle)
				End If
			End If
        ...
        Protected ReadOnly Property _Control() As XRDataContainerControl
            Get
                Return DirectCast(Control, XRDataContainerControl)
            End Get
        End Property
